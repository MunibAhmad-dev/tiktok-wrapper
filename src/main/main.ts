...
async function refreshFacebookAccountProfile(account: WorkspaceAccount, generation: number): Promise<void> {
  ...
  try {
      ...
      (() => {
        ...
        const isUsableName = (label) => {
          ...
          return value &&
            value.length >= 2 &&
            value.length <= 80 &&
            !/tiktok|instagram|home|menu|explore|reels|notifications|account|profile|search|create|direct/i.test(value);
        };
    ...
    const shouldReplaceLabel = ['TikTok Account', 'TikTok Profile', 'Instagram Account', 'Instagram Profile', 'App User', 'Facebook Account'].includes(account.label);
...
function scheduleFacebookProfileRefresh(account: WorkspaceAccount, delay = 2500): void {
  ...
  if (currentUrl && !currentUrl.includes('tiktok.com')) return;
...
function rebuildMenu(): void {
  ...
  const builder = new MenuBuilder(mainWindow, accountManager, store, showOrCreateMainWindow);
  builder.buildMenu();
}
...
function setupIPC(): void {
  ...
  // ── AI Post Studio — generate TikTok captions & hashtags (main process) ──────
  // kind: 'caption' → returns { text } (a catchy title/description for a post)
  //       'hashtags' → returns { hashtags: string[] } (relevant TikTok hashtags)
  ipcMain.handle('ai:generatePostContent', async (_e, topic: string, kind: 'caption' | 'hashtags', tone = 'friendly') => {
    ...
    const system = kind === 'hashtags'
      ? `You are a TikTok growth strategist. Generate 12 to 15 highly relevant, trending TikTok hashtags for the given post topic. Mix broad and niche tags. Return ONLY a JSON array of strings, each starting with "#", no markdown, no extra text.`
      : `You are a TikTok content strategist. Write one engaging TikTok post caption for the given topic in a ${tone} tone: a scroll-stopping hook line followed by a short description. Keep it under 150 characters, add 1-2 fitting emojis. Return ONLY the caption text — no quotes, no labels, no hashtags.`;
  ...
  ipcMain.handle('workspace:loadFacebook', (_e, workspaceId: string, accountId?: string) => {
    ...
    try {
      ...
      if (!account) {
        ...
        const newAccount = authManager.addWorkspaceAccount(workspaceId, 'TikTok Account');
      ...
      console.error('Failed to load TikTok:', err);
      return { success: false, error: 'Failed to load TikTok' };
    }
  });
