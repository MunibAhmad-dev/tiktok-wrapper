...
export function Sidebar({ }: SidebarProps) {
  ...
  return (
    <div className={cn(
      'h-screen bg-[#fdf8fc] dark:bg-[#0f0a12] border-r border-border/50 flex flex-col overflow-hidden transition-all duration-300 shrink-0',
      W
    )}>
      {/* Top bar */}
      <div className={cn('flex items-center border-b border-border/30 px-3 py-3 bg-background/60 backdrop-blur-xl', isMac && 'pt-10', isExpanded ? 'justify-between' : 'justify-center')}>
        {isExpanded && (
          <div className="flex items-center gap-2 min-w-0 flex-1 overflow-hidden">
            <img src={logoUrl} alt="App" className="w-7 h-7 rounded-xl object-cover shrink-0 shadow-sm" />
            <span className="text-[13px] font-semibold text-foreground truncate leading-tight">Apps for TikTok</span>
          </div>
        )}
        <button
        ...
        <div className={cn('mt-2', isExpanded ? 'pl-0' : 'px-0')}>
          {isExpanded && (
            <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider px-2 mb-1.5 flex items-center gap-1">
              Tools
              {!isPremium && <Sparkles className="h-2.5 w-2.5 text-[#FE2C55]" />}
            </p>
          )}
          {TOOL_ITEMS.map(item => (
            <NavButton
  ...
  return (
    ...
    >
      ...
      {isPremiumLocked && isExpanded && (
        <span className="text-[9px] font-bold text-[#FE2C55] bg-[#FE2C55]/10 dark:bg-[#FE2C55]/15 border border-[#FE2C55]/30 dark:border-[#FE2C55]/40 rounded-full px-1.5 py-0.5 shrink-0">
          PRO
        </span>
      )}
      {isPremiumLocked && !isExpanded && (
        <span className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-[#FE2C55] rounded-full flex items-center justify-center text-[7px] text-white font-bold">
          ✦
        </span>
      )}
    </button>
