...
export function Dashboard() {
  ...
  const handleAddAccount = async () => {
    ...
    const newAccount = await window.electronAPI?.workspaceAccount.add(wsId, 'TikTok Account')
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto bg-background">
      ...
      >
        ...
        <div className="flex items-center gap-2" style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
          {!isPremium && (
            <Button
              size="sm"
              className="gap-1.5 bg-[#FE2C55] hover:bg-[#E0234A] text-white text-xs border-none"
      ...
      <div className="flex-1 px-8 py-6 space-y-8 max-w-4xl mx-auto w-full">
          ...
          {accounts.length === 0 ? (
            <div className="rounded-2xl border-2 border-dashed border-border/60 p-10 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Add a TikTok account to start messaging
              </p>
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isLocked
                      ? 'bg-card/50 border-border/30 opacity-70 hover:opacity-90 hover:border-[#FE2C55]/40'
                ...
                >
                  ...
                  {isLocked && (
                    <span className="absolute top-2.5 right-2.5 text-[9px] font-bold text-[#FE2C55] bg-[#FE2C55]/10 dark:bg-[#FE2C55]/15 border border-[#FE2C55]/30 dark:border-[#FE2C55]/40 rounded-full px-1.5 py-0.5">
                      PRO
                    </span>
                  )}
                </button>
        ...
        {/* ── Tip banner ────────────────────────────────────────────────── */}
        <div className="rounded-2xl bg-gradient-to-r from-[#FE2C55]/10 to-[#25F4EE]/10 dark:from-[#FE2C55]/15 dark:to-[#25F4EE]/15 border border-[#FE2C55]/20 dark:border-[#FE2C55]/30 p-4 flex items-center gap-4">
