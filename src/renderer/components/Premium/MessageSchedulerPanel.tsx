...
export function MessageSchedulerPanel() {
  ...
  // We load TikTok in the background BrowserView and inject silently instead.
  const sendNow = async (item: ScheduledMessage) => {
    ...
    try {
      ...
      // Load TikTok in the background BrowserView (no view switch needed)
      ...
      if (result?.success) {
        ...
        toast.success('✅ Message injected into TikTok DM — press Enter in the chat to send!', { duration: 6000 })
      } else if ((result as any)?.error === 'no_conversation') {
        // Reset back to ready so user can try again after opening a conversation
        resetToReady()
        toast.warning(
          '📩 Please click on a conversation in TikTok messages first, then click Send Now again.',
          { duration: 8000 }
        )
      } else {
        const ok = await safeCopy(item.message)
        setItems(prev => prev.map(i => i.id === item.id ? { ...i, status: 'sent' } : i))
        toast.info(ok ? '📋 Copied to clipboard — paste it into your TikTok DM conversation.' : '⚠️ Could not inject or copy — please send the message manually.', { duration: 7000 })
      }
    } catch {
  ...
  return (
    <PremiumGate
      ...
      description="Schedule messages to be automatically injected into TikTok messages at a specific time."
      icon="📅"
    >
          ...
          <div className="rounded-2xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200/60 dark:border-blue-800/30 p-3.5 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                Schedule a message → a countdown runs → when time arrives you get a notification and a <strong>Send Now</strong> button appears → clicking it injects the text directly into TikTok messages → press <kbd className="px-1 py-0.5 rounded bg-background border border-border text-[10px] font-mono">Enter</kbd> to send.
              </p>
            </div>
          </div>
                    ...
                    <div className="flex flex-col gap-1.5 shrink-0">
                      {(isReady || isSending) && !isSent && (
                        <button
                          ...
                          title="Send now — injects text into TikTok messages"
                        >
