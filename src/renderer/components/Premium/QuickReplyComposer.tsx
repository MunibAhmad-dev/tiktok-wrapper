...
export function QuickReplyComposer() {
  ...
  const sendToMessenger = async () => {
    ...
    try {
      ...
      if (result?.success) {
        toast.success('✓ Text inserted — press Enter in TikTok to send', { duration: 4000 })
        setMessage('')
      } else if ((result as any)?.error === 'no_conversation') {
        toast.warning(
          '📩 Please click on a conversation in TikTok messages first, then click "Send to TikTok" again.',
          { duration: 7000 }
        )
        ...
        toast.info(ok ? '📋 Copied to clipboard — paste it into your TikTok DM conversation.' : '⚠️ Could not copy automatically — please copy the text manually.', { duration: 5000 })
      }
    } catch (err) {
      const ok = await safeCopy(message.trim())
      toast.info(ok ? '📋 Copied to clipboard — paste it into your TikTok DM conversation.' : '⚠️ Could not copy automatically — please copy the text manually.')
    } finally {
      setIsSending(false)
    }
  }
  ...
  return (
    <PremiumGate
      ...
      description="Compose replies natively and inject them directly into TikTok messages — no copy-paste needed."
      icon="✍️"
    >
      <div className="flex flex-col h-full">
        ...
        >
          ...
          <div style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
            ...
            <p className="text-[11px] text-muted-foreground">Injects text directly into TikTok DM</p>
          </div>
        </div>
          ...
          <div className="rounded-2xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200/60 dark:border-blue-800/30 p-3.5 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                Type your reply below → click <strong>Send to TikTok</strong> → the text is inserted directly into the active DM input. Just press <kbd className="px-1 py-0.5 rounded bg-background border border-border text-[10px] font-mono">⏎ Enter</kbd> to send it.
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            ...
            >
              {isSending
                ...
                : <><Send className="h-4 w-4" /> Send to TikTok</>}
            </Button>
