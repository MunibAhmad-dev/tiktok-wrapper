    ...
    try {
      ...
      if (ws.length > 0) {
        ...
        if (accts.length > 0) {
          ...
          const a = await window.electronAPI?.workspaceAccount.add(ws[0].id, 'TikTok Account')
          if (a && !('error' in a)) {
        ...
        if (newWs) {
          setWorkspaces([newWs])
          const a = await window.electronAPI?.workspaceAccount.add(newWs.id, 'TikTok Account')
          if (a && !('error' in a)) {
            setWorkspaceAccounts([a])
            setActiveWorkspaceAccountId(a.id)
